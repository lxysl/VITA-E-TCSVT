...
class Gr00tPolicy(BasePolicy):
    ...
    def get_hidden_states(self, observations: Dict[str, Any]) -> torch.Tensor:
        """
        Get hidden states from the model without generating actions.
        
        Args:
            observations (Dict[str, Any]): The observation to process.
            
        Returns:
            torch.Tensor: Hidden states from the backbone model.
        """
        ...
        with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=COMPUTE_DTYPE):
            hidden_states = self.model.get_hidden_states(normalized_input)
